...
void ACameraDriver::BeginPlay() {
    ...
    // 4. Connect 
    video_manager_->Connect(IPAddress, Port+1);
	...
	for (FAssetData LevelSequenceAsset : LevelSequenceAssetList)
	{
		...
		level_sequence_editor_ = (FLevelSequenceEditorToolkit*)AssetEditor;
		 if (level_sequence_editor_ != nullptr)
		 {
			 // Get current Sequencer
			 ISequencer* Sequencer = level_sequence_editor_->GetSequencer().Get();
			 Sequencer->OnPlay();
		 }
	 }
	 */
